...
class RenameProviderImpl {
    ...
    getRenameInfo(lang, tsDoc, doc, originalPosition, generatedOffset) {
        ...
        if (this.configManager.getConfig().svelte.useNewTransformation &&
            ((0, documents_1.isInHTMLTagRange)(doc.html, doc.offsetAt(originalPosition)) ||
    ...
    async getAdditionLocationsForRenameOfPropInsideComponentWithProp(document, tsDoc, position, convertedRenameLocations, snapshots, lang) {
        ...
        const regex = new RegExp(`export\\s+let\\s+${this.getVariableAtPosition(tsDoc, lang, position)}($|\\s|;|:)` // ':' for typescript's type operator (`export let bla: boolean`)
        );
    ...
    checkShortHandBindingOrSlotLetLocation(lang, renameLocations, snapshots) {
        ...
        return renameLocations.map((location) => {
            ...
            const newRange = (0, documents_1.mapRangeToOriginal)(snapshot, (0, utils_2.convertRange)(snapshot, {
    ...
    getShorthandPropInfo(sourceFile, location) {
        var _a;
        const possibleJsxAttribute = (0, utils_3.findContainingNode)(sourceFile, location.textSpan, typescript_1.default.isJsxAttribute);
    ...
    getSlotLetInfo(sourceFile, location) {
        const possibleSlotLet = (0, utils_3.findContainingNode)(sourceFile, location.textSpan, typescript_1.default.isVariableDeclaration);
        if (!possibleSlotLet || !typescript_1.default.isObjectBindingPattern(possibleSlotLet.name)) {
            return null;
        }
        const bindingElement = (0, utils_3.findContainingNode)(possibleSlotLet.name, location.textSpan, typescript_1.default.isBindingElement);
